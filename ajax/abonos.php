...
switch ($_GET["op"]){
    ...
    $row =oci_fetch_assoc($resp);
      $totalemision=$row['EMISION'];
      $totalabono=$row['VAL_ABONOS'];
     ...
     case 'totalesPediosUrbanos':
     $texto=$_POST['texto'];
     $resp=$deudas->totalesPediosUrbanos($texto);
     $totalemision=0;
     $totalabono=0;
     $totalinteres=0;
    $row =oci_fetch_assoc($resp);
     ...
     case 'totalesagua_potable':
     $texto=$_POST['texto'];
     $resp=$deudas->totalesagua_potable($texto);
     $totalemision=0;
     $totalabono=0;
     $totalinteres=0;
    $row =oci_fetch_assoc($resp);
     ...
     case 'totalespatente_municipal':
     $texto=$_POST['texto'];
     $resp=$deudas->totalespatente_municipal($texto);
     $totalemision=0;
     $totalabono=0;
     $totalinteres=0;
    $row =oci_fetch_assoc($resp);
     ...
     while ($row =oci_fetch_assoc ($resp)){
      $str = str_replace(',','.',$row['EMISION']);
       $data[]=array(
         '0'=>'<a class="btn-circle btn btn-orange text-white" title="Pagar servicio en línea" onclick="pagar('.$row['NROEMISION'].','.$str.')">PL</a> <a class="btn-circle btn btn-cyan text-white popover-item" title="Detalle de impuesto" onclick="detalles('.$row['NROEMISION'].')">DE</a>',
         '1'=>$row['NROEMISION'],
         '2'=>$row['CIU'],
         '3'=>$row['CEDULA_RUC'],
         '4'=>$row['NOMBRES'],
    ...
    case 'buscar_predios_urbanos':

    $texto=$_GET['text'];

    $resp=$deudas->buscar_predios_urbanos($texto);
    $data= Array();
    ...
    break;
    case 'buscar_agua_potable':

    $texto=$_GET['text'];

    $resp=$deudas->buscar_agua_potable($texto);
    $data= Array();
    ...
    break;
    case 'buscar_patente_municipal':

    $texto=$_GET['text'];

    $resp=$deudas->buscar_patente_municipal($texto);
    $data= Array();
